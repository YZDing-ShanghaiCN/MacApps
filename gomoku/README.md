...
看到 `Uvicorn running on http://127.0.0.1:8000` 后，不要关闭这个终端。
...
在 **Mac 和手机** 上都打开这个 `trycloudflare.com` 地址；随后在 Mac 的这个公网页面点击“创建私人房间”，再把新生成的完整邀请链接发给对方。不要从 `http://127.0.0.1:8000` 页面创建房间，也不要发送旧的 `127.0.0.1` 链接。

两个终端都必须持续运行；停止其中任一个，远程对战都会中断。每次重启终端 2，Cloudflare 都会生成一个新地址，因此要重新从新地址创建房间并分享新的邀请链接。
