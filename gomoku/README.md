...
在 Mac 浏览器打开 [http://127.0.0.1:8000](http://127.0.0.1:8000)。网页支持本地双人、人机、胜利棋线高亮、黑白双方累计用时、悔棋和重开。宽屏时黑白双方的信息、身份、状态和计时分列在棋盘两侧，公共状态与操作集中在棋盘上方；窄屏自动切换为棋盘上方的双玩家卡片。人机模式可选择人类执黑、执白或随机棋色；AI 执黑时会在开始对局后自动走第一手。
...
- “困难”使用战术引擎 + 蒙特卡洛树搜索的 HardAI：先验证一步获胜与必须阻挡，再依次运行 VCF（连续冲四）与 VCT（连续做杀）证明搜索，只有在防守方所有应对（阻挡、反胜、反四，全量枚举不截断）都被验证必败时才宣布强制胜；随后对对手的强制胜链做逐点验证的战术防守，最后用剩余时间跑确定性 PUCT 蒙特卡洛树搜索兜底。所有阶段共享同一绝对时限（逐模拟、逐探测点检查），超时结果永远不会被当作证明或落子使用。策略/价值默认来自启发式评估（未训练模型）；可选的自博弈训练流水线（竞技场基线 → 自博弈数据生成 → 小型策略-价值网络 → 只保留更强模型的迭代循环）见 [docs/selfplay_training.md](docs/selfplay_training.md)：训练脚本支持流式回放缓冲（`--replay-window`/`--replay-max-records`），自博弈可用 `--root-noise` 加根 Dirichlet 噪声（仅自博弈，真实对局始终确定），晋级判定只用 Wilson 95% 区间下界。已有训练模型无需改代码即可接入：`export GOMOKU_HARD_AI_MODEL_PATH=/path/to/model.pt`（路径失效自动回退启发式并在调试快照中说明）。全部参数集中在 `src/gomoku/ai/hard_ai_config.py`，渐进式扩展、模型全局 Top-K、跨回合根复用与叶子战术等技术细节见 [docs/hard_ai.md](docs/hard_ai.md)。
