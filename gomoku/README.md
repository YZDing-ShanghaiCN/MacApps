...
- “普通”使用纯搜索 NormalAI：迭代加深 Negamax、Alpha-Beta 剪枝、固定种子增量 Zobrist 哈希、置换表、棋型静态评估、威胁优先候选点和严格的每步时间预算。NormalAI 能识别连续及带间隔的四、三、二等棋形，默认最多搜索 800ms，并始终保留一步获胜和必须阻挡的落点。

“困难”难度仍在开发中。第一版 NormalAI 不包含 VCF/VCT、强化学习、神经网络或外部模型/API。
