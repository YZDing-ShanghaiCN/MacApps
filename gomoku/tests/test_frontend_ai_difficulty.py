...
def test_frontend_enables_all_difficulty_buttons() -> None:
    ...
    assert hard_tag is not None and "disabled" not in hard_tag.group(0)
    assert "困难" in hard_tag.group(0)
...
def test_frontend_posts_selected_difficulty() -> None:
    ...
    assert 'changeDifficulty("hard")' in javascript
