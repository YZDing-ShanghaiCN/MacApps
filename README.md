...
- **[sgame](./sgame)** — 六款纯前端小游戏合集：舒尔特方格、扫雷、吃豆人、2048、俄罗斯方块、24点，浏览器点开即玩，支持 PWA 离线访问。
...
| 主菜单 | 游戏界面 |
|:---:|:---:|
| ![24点主菜单](docs/screenshots/24point-menu.png) | ![24点游戏界面](docs/screenshots/24point-game.png) |
...
在浏览器打开 <http://127.0.0.1:8001>，从首页选择游戏进入。六个游戏均为纯前端实现，无需后端存储，成绩与进度保存在浏览器本地。
...
经典 24 点数字谜题：给出 4 个随机整数，用 +、−、×、÷ 和括号把它们组成算式，使结果等于 24，每个数字恰好使用一次。简单模式数字范围 1–13，困难模式 1–99；中间结果可以是分数（例如 8÷(3−8÷3)=24）。每道题都保证有解（发牌前先由内置求解器验证），输入算式回车或点击「验证」检查结果，「换一题」随时重新发牌。
