...
- “困难”使用战术引擎 + 蒙特卡洛树搜索的 HardAI：先验证一步获胜与必须阻挡，再依次运行 VCF（连续冲四）与 VCT（连续做杀）证明搜索，只有在防守方所有应对（阻挡、反胜、反四，全量枚举不截断）都被验证必败时才宣布强制胜；随后对对手的强制胜链做逐点验证的战术防守，最后用剩余时间跑确定性 PUCT 蒙特卡洛树搜索兜底。所有阶段共享同一绝对时限（逐模拟、逐探测点检查），超时结果永远不会被当作证明或落子使用。目前策略/价值来自启发式评估，未训练模型；强化学习或神经网络模型未来通过 `PolicyValueProvider` 接口接入。全部参数集中在 `src/gomoku/ai/hard_ai_config.py`，技术细节见 [docs/hard_ai.md](docs/hard_ai.md)。
